<script lang="ts">
    ...
    import { opLists, type FaceType, type Operator, type OpType } from '../types/Operator';
  ...
  let attrMenuEl; // メニューDOM参照
	...
	// ===== フィールド関連 =====
	let fields = Array.from({ length: gridH }, () =>
	...
	const FieldUtils =  {
		...
		clearField() {
			fields = Array.from({ length: gridH }, () =>
				Array.from({ length: gridW }, () => 0) // 0: empty
			);
		},
		// コピー&ペースト操作
		copyPaste: {
			...
			updateSelRect() {
				if (selStart && selEnd) selRect = FieldUtils.copyPaste.rectFromPoints(selStart, selEnd);
			},
			...
			clearSelection() {
				isSelecting = false; selStart = null; selEnd = null; selRect = null;
			},
			onCellPointerDownSelect(e: PointerEvent & { currentTarget: EventTarget & HTMLButtonElement; }, x: any, y: any) {
			...
			copySelection() {
				...
				const bufFields = Array.from({ length: h }, (_, dy) =>
					Array.from({ length: w }, (_, dx) => FieldUtils.getField(x + dx, y + dy))
				);

				const bufAttrs  = Array.from({ length: h }, (_, dy) =>
					Array.from({ length: w }, (_, dx) => FieldUtils.attrChange.getAttr(x + dx, y + dy))
				);
			...
			pasteSelectionAt(x: number, y: number) {
				...
				const bufFields = Array.isArray(copyBuffer) ? copyBuffer : copyBuffer.fields;
				const bufAttrs  = Array.isArray(copyBuffer) ? null       : (copyBuffer.attrs ?? null);
					...
					for (let dx = 0; dx < w; dx++) {
						...
						if (bufAttrs) {
							...
							if (FieldUtils.attrChange.isAttrApplicable(srcField)) {
								...
								// 属性なしの場合：クリア
								if (attrs[ty]) attrs[ty][tx] = FieldUtils.attrChange.isAttrApplicable(srcField) ? attrs[ty][tx] : 0;
						}
					}
				}
				// 変更検知
	...
	const OpeUtils = {
		...
		// ===== 攻撃範囲（パターン＋回転） =====
		basePattern(name: string) {
			let ofs: number[][] = [];
			if (!name) return ofs;

			if (name.startsWith('line')) {
				const n = parseInt(name.split(':')[1] || '1', 10);
				ofs = Array.from({ length: n }, (_, i) => [0, -(i + 1)]);
			}	else if (name.startsWith('diamond')) {
				const r = parseInt(name.split(':')[1] || '1', 10);
				for (let dx = -r; dx <= r; dx++) {
					for (let dy = -r; dy <= r; dy++) {
						if ((Math.abs(dx) + Math.abs(dy) <= r) && (dx || dy)) ofs.push([dx, dy]);
					}
				}
			}
			return ofs;
		...
		attackTiles(op:Operator) {
			...
			const patternName = skills[op.skillIdx ?? 0]?.pattern;
			const base = OpeUtils.basePattern(patternName);
			return base
		...
		onCellClick(x:number, y:number) {
			if (activeLayer === 'field') {
				...
				const op = OpeUtils.opAt(x, y);
				if (op) OpeUtils.toggleFacingAt(x, y); // 中心マスクリックで向き切替
				else OpeUtils.placeOperator(x, y);     // 何もなければ配置
			}
		},
		// ===== 移動操作 =====
		move: {
			onOpPointerDown(e: PointerEvent & { currentTarget: EventTarget & HTMLDivElement; }, op: Operator) {
				draggingId = op.id;
			...
			onGridPointerUp(e: { pointerId: number; }) {
				if (draggingId == null) return;
				// ドロップ先セルへスナップ
				const gx = Math.min(gridW - 1, Math.max(0, Math.floor(dragPx.x / CELL)));
				const gy = Math.min(gridH - 1, Math.max(0, Math.floor(dragPx.y / CELL)));
				if (!OpeUtils.opAt(gx, gy)) {
					operators = operators.map(o => o.id === draggingId ? { ...o, x: gx, y: gy } : o);
				}
				draggingId = null;
				if (isSelecting) {
					isSelecting = false;
			...
			opPixel(op: Operator) {
				if (draggingId === op.id) return { left: dragPx.x - CELL / 2, top: dragPx.y - CELL / 2 };
	...
	function bgOfField(idx: number) { return fieldTypes[idx]?.color ?? '#fff'; }
	...
	const MenuUtils = {
		...
		handleKeyDown(e: { key: string; metaKey: any; ctrlKey: any; preventDefault: () => void; }) {
			...
			if (activeLayer === 'field') {
				if (isMeta && key === 'c') {
					if (selRect) { FieldUtils.copyPaste.copySelection(); FieldUtils.copyPaste.enterPasteMode();  e.preventDefault(); }
				} else if (key === 'escape') {
					FieldUtils.copyPaste.cancelPasteMode();
		...
		onCellContextMenu(e: MouseEvent & { currentTarget: EventTarget & HTMLButtonElement; }, x: number, y: number) {
...
<div class="maparea">
	...
	<div class="toolbar">
		<div class="group">
			...
			<label><input type="radio" bind:group={activeLayer} value="field"> フィールド</label>
			<label><input type="radio" bind:group={activeLayer} value="operator"> オペレーター</label>
		</div>
		<div class="child-group">
			{#if activeLayer === 'field'}
				...
				<div>
						<label>横
							<select bind:value={selW} on:change={(e) => resizeGrid(+selW, gridH)}>
								{#each sizeOptions as n}<option value={n}>{n}</option>{/each}
							</select>
					</label>
						×
						<label>縦
							<select bind:value={selH} on:change={(e) => resizeGrid(gridW, +selH)}>
									{#each sizeOptions as n}<option value={n}>{n}</option>{/each}
							</select>
					</label>
					</div>
				...
				<div class="group">
					<div class="hint copyinfo">・コピー&ペースト：Shift＋ドラッグで範囲選択 → Ctrl + C → コピー先のセルをクリック</div>
					<div class="hint copyinfo">・マス情報追加：地上マス、高台マスで右クリック</div>
				</div>
			{:else}
				<div class="group">
		...
		>
			...
			{#each Array(gridH) as _, y}
				{#each Array(gridW) as __, x}
					<button
						...
						on:pointerdown={(e) => FieldUtils.copyPaste.onCellPointerDownSelect(e, x, y)}
						on:click={() => {
								// ★貼り付けモードならここが最優先
			...
			{#each operators as op}
				...
				<div
					class="op"
					style={
						`left:${OpeUtils.move.opPixel(op).left}px; 
					...
					on:contextmenu={(e) => { e.preventDefault(); e.stopPropagation(); OpeUtils.cycleSkill(op); }}
					title={`${op.type.label} / 向き: ${op.facing}`}
				>
					<div class="op-body">
						<button
							class="op-del"
							on:click|stopPropagation|preventDefault={() => OpeUtils.removeOperator(op.id)}
							on:pointerdown|stopPropagation
							title="このオペレーターを削除"
							aria-label="オペレーターを削除"
						>×</button>
						<div class="op-type">{op.type.label}</div>
			...
			{#if attrMenu.open}
				...
				>
					...
					<label class="row">
						<input
							type="checkbox"
							checked={(FieldUtils.attrChange.getAttr(attrMenu.gx, attrMenu.gy) & ATTR_BLOCK) !== 0}
							on:change={(e) => FieldUtils.attrChange.toggleAttrBit(attrMenu.gx, attrMenu.gy, ATTR_BLOCK, e.currentTarget.checked)}
						/>
						配置不可
					</label>
					<label class="row">
						<input
							type="checkbox"
							checked={(FieldUtils.attrChange.getAttr(attrMenu.gx, attrMenu.gy) & ATTR_EFF) !== 0}
							on:change={(e) => FieldUtils.attrChange.toggleAttrBit(attrMenu.gx, attrMenu.gy, ATTR_EFF, e.currentTarget.checked)}
						/>
						効果
					</label>
					<div class="buttons">
						<button class="mini" on:click={() => (attrMenu = { open:false, gx:0, gy:0, left:0, top:0 })}>閉じる</button>
						<button class="mini subtle" on:click={() => { FieldUtils.attrChange.setAttr(attrMenu.gx, attrMenu.gy, 0); }}>クリア</button>
					</div>
				</div>
			{/if}
		</div>
...
<style>
	...
	* { box-sizing: border-box; }
	...
	.group { display: flex; column-gap: 10px; width: 100%; align-items: center; flex-wrap: wrap;	}
	.group .head { font-weight: 700; margin-right: 10px; }
	.hint { font-size: 12px; color: #ffffff;  width: 100%; }
	.hint.copyinfo { font-size: 12px; color: #ffffff; }
	
	/** ----------------------- グリッド----------------------- */
	.stage-wrap { overflow: auto; padding: 20px; border: 1px dashed #d4d4d8; border-radius: 12px; }
	.grid {
		position: relative;
		display: grid;
	...
	.cell:hover { outline-color: #3b82f6aa; }
	.cell .tag {
		position: absolute; right: 2px; top: 2px;
		font-size: 10px; padding: 1px 4px; border-radius: 6px;
	...
	.attr-chip.block { left: 2px;  background: #ef4444; } /* 赤: 配置不可 */
	.attr-chip.eff   { right: 2px; background: #10b981; } /* 緑: 効果 */
	...
	.attr-menu .mini.subtle { color: #374151; }
	.attr-menu .mini:hover { background: #f3f4f6; }
	.range {
		position: absolute;
	...
	.op .op-del:hover { background: #fee2e2; }
	...
	.op-type { font-size: 10px; opacity: 0.8; }
	.op-arrow { font-size: 18px; line-height: 1; }
	.op-skill {
		position: absolute;
		top: -5px; left: -5px;
		font-size: 10px;
	...
	.legend {
		display: flex; gap: 12px; margin: 10px 0 4px; flex-wrap: wrap;
		color: #374151; font-size: 14px;
	}
	.legend .swatch {
		display: inline-block; width: 12px; height: 12px;
		background: var(--c); border: 1px solid #0002; border-radius: 3px; margin-right: 6px;
	}

	@media (prefers-color-scheme: dark) {
		.stage-wrap { border-color: #52525b; }
		.cell { outline-color: #030000; }
		.legend { color:  white }
		.op-skill { background: #1d4ed8; }
		.attr-chip.block { background: #b91c1c; }
		.attr-chip.eff   { background: #059669; }
		.attr-menu {
			background: #0b0b0c;
			border-color: #3f3f46;
			box-shadow: 0 8px 24px rgba(0,0,0,.45);
		}
		.attr-menu .mini { background: #0b0b0c; border-color: #3f3f46; color: #e5e7eb; }
		.attr-menu .mini:hover { background: #111827; }
	}
</style>
